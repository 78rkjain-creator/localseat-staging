    ...
    <div className="px-4 sm:px-6 py-8 max-w-6xl mx-auto">
      ...
      {activeTag && (
        <div className="flex items-center gap-2 mb-4">
          ...
          <Link href="/voter-list" className="text-sm text-brand-600 hover:text-brand-700 font-medium">
            Clear
          </Link>
        </div>
      )}
      ...
      {!tag && allTags.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-5">
          {allTags.map((t) => (
            <Link key={t.id} href={`/voter-list?tag=${t.id}`} className="hover:opacity-80 transition-opacity">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {page > 1 ? (
                  <Link
                    href={pageUrl(page - 1)}
                    className="h-9 px-4 rounded-xl border border-slate-200 text-sm font-medium text-slate-700 hover:bg-slate-50 inline-flex items-center transition-colors"
                  >
                    Previous
                  </Link>
                ) : (
