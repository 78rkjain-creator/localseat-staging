    ...
    <div className="px-4 sm:px-6 py-8 max-w-6xl mx-auto">
      ...
      {activeTag && (
        <div className="flex items-center gap-2 mb-4">
          ...
          <Link href="/voter-import" className="text-sm text-brand-600 hover:text-brand-700 font-medium">
            Clear
          </Link>
        </div>
      )}
      ...
      {!tag && allTags.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-5">
          {allTags.map((t) => (
            <Link key={t.id} href={`/voter-import?tag=${t.id}`} className="hover:opacity-80 transition-opacity">
